...
export default function ImageSlider() {
  ...
  // manual sroll navigation functions
  const scroll = (direction) => {
    if (sliderRef.current) {
      ...
      sliderRef.current.scrollTop({
        left: direction === "left" ? scrollLeft : scrollLeft + scrollAmount,
      });
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-2">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-neutral-900 border border-neutral-800 text-xs text-neutral-400 font-body tracking-wdiset uppercase">
              <FiCamera className="w-3.5 h-3.5 text-amber-400" />
              <span>Recent Highlights</span>
            </div>
            <h2 className="text-3x; md:text-4xl font-bold tracking-tight">
        ...
        <div
          ...
          {sliderImage.map((img) => (
            <div
              key={img.id}
              className="snap-start shrink-0 w-70 sm:w-[320px] md:w-95 h-110 relative rounded-3xl overflow-hidden group border-neutral-800/80 bg-neutral-900/50 transition-transform duration-300 hover:-translate-y-1.5 select-none"
              onContextMenu={(e) => e.preventDefault()} // image protection
            >
              <img
                src={img.imageUrl}
                alt={img.title}
              ...
              {/* dark gradient overlay for typhography readability */}
              <div className="absolute inset-0 bg-linear-to-t from-neutral-950 via-neutral-950/20 to-tran] opacity-80 group-hover:opacity-90 transition-opacity"></div>

              {/* content overlay */}
              <div className="absolute bottom-0 left-0 right-0 p-6 space-y-2">
                <span className="inline-block px-3 py-1 rounded-full bg-white/10 backdrop-blur-md text-[10px] font-heading uppercase tracking-widset text-amber-300 border border-white/10">
                  {img.category}
                </span>
